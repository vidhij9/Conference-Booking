...
public class CancelCommandExecutor extends CommandExecutor {
  public static String COMMAND_NAME = "CANCEL";

  public CancelCommandExecutor(List<Building> buildingList) {
    super(buildingList);
  }
  ...
  public void execute(Command command) {
    ...
    for (Slot slot : room.getSlotList()) {
      if (slot.getStartTime() == startTime && slot.getEndTime() == endTime) {
        room.getSlotList().remove(slot);
        break;
      }
    }
  }
